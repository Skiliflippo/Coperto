...
export async function GET(req: Request) {
  try {
    ...
    // Con lo slug si serve SOLO quel locale: nessuna fuga di nomi fra ristoranti.
    const [restaurant] = slug
      ? await db.select().from(s.restaurants).where(eq(s.restaurants.slug, slug)).limit(1)
      : await db.select().from(s.restaurants).orderBy(asc(s.restaurants.createdAt)).limit(1);
    if (!restaurant) {
