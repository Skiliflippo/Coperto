...
import { useTenant, useTenantPath, withSlug } from "@/lib/tenant";
...
export default function SetupPage() {
  ...
  useEffect(() => {
    api<{ needsSetup: boolean }>(withSlug(slug, "/api/setup"))
      .then((d) => { if (!d.needsSetup) router.replace(tp("/login")); else setChecking(false); })
      .catch((e: unknown) => {
  ...
  const create = async () => {
    ...
    try {
      const session = await api<StaffSession>("/api/setup", {
        method: "POST",
        body: { restaurantName, ownerName, pin, slug },
      });
      setStaff(session);           // entra già come titolare
      router.replace(tp("/sala"));     // da qui parte il wizard della piantina
    } catch (error: unknown) {
