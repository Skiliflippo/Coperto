"use client";
// Primitive di disegno della piantina — stabili, senza crash iOS.
// Un solo layer GPU: floor-content. Niente will-change su tavoli/muri per evitare
// esaurimento memoria GPU su Safari ("a problem repeatedly occurred").
...
export function GridBackdrop({
  ...
  // Grid ottimizzata: buffer 300px + transform modulo major per evitare repaint e buchi
  // major max ~240px (zoom 2.4), buffer 300 copre sempre anche con pan veloce
  // Transform GPU-only, niente backgroundPosition che triggera paint su iPad
  const mod = (n: number, m: number) => ((n % m) + m) % m;
  const gx = mod(vp.panX, major) - major;
  const gy = mod(vp.panY, major) - major;
  return (
    <div
      ref={ref}
      className="floor-grid pointer-events-none absolute"
      style={{
        left: -300,
        top: -300,
        ...
        transform: `translate3d(${gx}px, ${gy}px, 0)`,
      }}
    />
  );
}
  ...
  >(function TableNode(
    ...
    return (
      ...
      >
        ...
        <div
          className={`h-full w-full border-[7px] bg-surface shadow-md ${tone} ${selected ? "!border-brand" : ""} ${invalid ? "!border-dashed !border-over bg-over/15" : ""}`}
        ...
        >
          ...
          >
            ...
            {sub && (
              <span className="mt-1.5 font-sans font-bold leading-none text-muted" style={{ fontSize: fs * 0.5 }}>
                {sub}
              </span>
            )}
            {dotClass && (
...
export const ElementNode = memo(
  ...
  >(function ElementNode({ el, selected, editable, invalid, hideLabel, onPointerDown, children }, ref) {
